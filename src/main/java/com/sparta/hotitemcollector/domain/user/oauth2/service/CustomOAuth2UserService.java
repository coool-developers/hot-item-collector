...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
     ...
     */
    private OAuthUser getUser(OAuthAttributes attributes, SocialType socialType) {
        OAuthUser findUser = oAuthUserRepository.findBySocialTypeAndEmail(socialType,
                attributes.getOauth2UserInfo().getEmail()).orElse(null);

        if(findUser == null) {
            return saveOAuthUser(attributes, socialType);
        }
        return findUser;
    }
     ...
     */
    private OAuthUser saveOAuthUser(OAuthAttributes attributes, SocialType socialType) {
        OAuthUser createdUser = attributes.toOAuthEntity(socialType, attributes.getOauth2UserInfo());
        return oAuthUserRepository.save(createdUser);
    }
}
